analyze.abmld2=function(h=10,atTime=NULL,atLag=NULL){
  ...
  abmld2.f.rec=forecastLSW::forecast.lpacf(abmld2[1:(length(abmld2)-h)],h=h,forecast.type='recursive')
  abmld2.acc.rec=forecastLSW::forecast.panel(abmld2.f.rec,truth=abmld2[(length(abmld2)-h+1):length(abmld2)],xlab='Year',xaxt='n',lwd=3,move=-0.05,col='blue')
  axis(side=1,at=1:h,labels=seq(1956,2010.75,by=0.25)[(length(abmld2)-h+1):length(abmld2)])
  
  abmld2.f.fix=forecastLSW::forecast.lpacf(abmld2[1:(length(abmld2)-h)],h=h,forecast.type='fixed')
  abmld2.acc.fix=forecastLSW::forecast.panel(abmld2.f.fix,truth=abmld2[(length(abmld2)-h+1):length(abmld2)],xlab='Year',xaxt='n',lwd=3,move=-0.1,col='red',add=TRUE)

  abmld2.f.ex=forecastLSW::forecast.lpacf(abmld2[1:(length(abmld2)-h)],h=h,forecast.type='extend')
  abmld2.acc.ex=forecastLSW::forecast.panel(abmld2.f.ex,truth=abmld2[(length(abmld2)-h+1):length(abmld2)],xlab='Year',xaxt='n',lwd=3,move=0.05,col='green',add=TRUE)
  ...
  abmld2.acc2=forecastLSW::forecast.panel(abmld2.f2,truth=abmld2[(length(abmld2)-h+1):length(abmld2)],add=TRUE,col='orange',lwd=2,move=0.1)
  
  title(main='ABMLd2 forecasts: red=fixed, blue=recursive, green=extend, orange=arma')
  #dev.off()
  
  return(list(lpacf=abmld2.lpacf,forecast.lpacf=list(fix=abmld2.f.fix,ex=abmld2.f.ex,rec=abmld2.f.rec),
